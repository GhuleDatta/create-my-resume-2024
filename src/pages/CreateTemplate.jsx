...
const CreateTemplate = () => {
            ...
            uploadTask.on(
                ...
                () => {
                    getDownloadURL(uploadTask.snapshot.ref).then(downloadURL => {
                        setImageAsset(prevAsset => ({ ...prevAsset, uri: downloadURL, isImageLoading: false }));
    ...
    const pushToCloud = async () => {
        ...
        const _doc = {
            _id: id,
            title: formData.title,
            imageURL: imageAsset.uri || "", // Change imageAsset.url to imageAsset.uri
            tags: selectedTags,
            name: templates && templates.length > 0 ?
                `Template${templates.length + 1}` :
                "Template1",
    ...
    return (
        <div className="w-full px-4 lg:px-10 2xl:px-32 py-4 grid grid-cols-1 lg:grid-cols-12">
            ...
            <div className="col-span-12 lg:col-span-4 2xl:col-span-3 w-full flex-1 flex items-center justify-start flex-col gap-4 px-2">
                        ...
                        <React.Fragment>
                            ...
                            {!imageAsset.uri ? (
                                ...
                                <React.Fragment>
                                    ...
                                    <img src={imageAsset.uri} className="w-full h-full object-cover" loading="lazy" alt="Uploaded" />
                                    {/* Delete action */}
                                    <div
                ...
                {/* button section */}
                <botton type="button" className="w-full bg-blue-800 text-white rounded-lg py-3 flex items-center justify-center shadow-md hover:bg-purple-900 transition duration-300 ease-in-out"
                onClick={pushToCloud}>
                    Save
                </botton>
            </div>
...
//     // handaling the input field change
//     const handleInputChange = (e) =>{
//         const{name, value} = e.target
//         setFormData((prevRec) => ({...prevRec, [name] : value}))
//     };
...
//             <div className="w-full">
//                 <p className="text-lg text-txtPrimary">Create a nee Template</p>
//             </div>
...
//                                 justify-center cursor-pointer flex-col gap-4">
//                                     <FaUpload className="text-2xl"/>
//                                     <p className="text-lg text-txtLight">Click to Upload</p>
//                                 </div>
//                             </div>
...
//          <div className="w-full flex items-center flex-wrap gap-2">
//             {initialTags.map((tag, i) => (
//                 <div tag={i} className={`border border-gray-300 px-2 py-1
