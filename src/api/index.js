    ...
    return new Promise((resolve, reject) => {
        const unsubscribe = auth.onAuthStateChanged((userCred)=>{
            if(userCred){
                ...
                //make sure to unsubscribe from the listener to provenet the memory leak
                unsubscribe()
        })
    })
}
...
export const getSavedResumes =(uid)=>{
    return new Promise((resolve, reject)=>{
        const templateQuery = query(
            collection(db,"users",uid,"resumes",orderBy("timeStamp","asc"))
        );
        ...
        return unsubscribe;//clean up the listener when unsbscribe
    });
};